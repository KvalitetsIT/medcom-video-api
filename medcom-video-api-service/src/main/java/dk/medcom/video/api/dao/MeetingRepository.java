...
public interface MeetingRepository extends CrudRepository<Meeting, Long> {
	List<Meeting> findByOrganisationCodeAndStartTimeBetween(String organisationCode, Date startTime, Date endTime);
	...
	List<Meeting> findByOrganisationCodeAndSubject(String organisationCode, String subject);

	List<Meeting> findByOrganisationCodeAndOrganizedBy(String organisationCode, MeetingUser organizedBy);

	List<Meeting> findByOrganizedBy(MeetingUser organizedBy);

	@Query("select s.meeting from SchedulingInfo s inner join s.meeting m where s.uriWithDomain = ?2 and m.organisationCode = ?1")
	List<Meeting> findByUriWithDomainAndOrganisationCode(String organisationCode, String uriWithDomain);
	...
	@Query("select s.meeting from SchedulingInfo s inner join s.meeting m where s.uriWithDomain = ?2 and m.organisationCode = ?1")
	Meeting findOneByUriWithDomainAndOrganisationCode(String organisationCode, String uriWithDomain);

	@Query("select s.meeting from SchedulingInfo s inner join s.meeting m where s.uriWithDomain = ?2 and m.organizedBy = ?1")
	Meeting findOneByUriWithDomainAndOrganizedBy(MeetingUser organizedBy, String uriWithDomain);

	@Query("select s.meeting from SchedulingInfo s inner join s.meeting m where s.uriWithoutDomain = ?2 and m.organisationCode = ?1")
	Meeting findOneByUriWithoutDomainAndOrganisationCode(String organisationCode, String uriWithDomain);
	...
	@Query("select l.meeting from MeetingLabel l inner join l.meeting m where l.label = ?2 and m.organisationCode = ?1")
    List<Meeting> findByLabelAndOrganisationCode(String organisationCode, String label);
	...
	@Query("select m from Meeting m where m.organisationCode = ?1 and (m.subject like ?2 or m.description like ?3)")
	List<Meeting> findByOrganisationCodeAndSubjectLikeOrDescriptionLike(String organisationCode, String subject, String description);
