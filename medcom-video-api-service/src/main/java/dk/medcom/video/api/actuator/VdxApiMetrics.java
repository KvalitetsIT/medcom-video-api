...
public class VdxApiMetrics extends PrometheusScrapeEndpoint {
	...
	public VdxApiMetrics(CollectorRegistry collectorRegistry, PoolInfoService poolInfoService) {
		...
		this.wantedPoolSize = Gauge.build().name("wanted_poolsize").help("Size of the wanted pool").labelNames(ORG_ID_KEY).register(collectorRegistry);
	...
	private void setPoolGauges() {
		...
		for (PoolInfoEntity poolInfo : poolInfos) {
			wantedPoolSize.labels(poolInfo.getOrganisationCode()).set(poolInfo.getWantedPoolSize());
			availablePoolSize.labels(poolInfo.getOrganisationCode()).set(poolInfo.getAvailablePoolSize());
		}
	}
}
