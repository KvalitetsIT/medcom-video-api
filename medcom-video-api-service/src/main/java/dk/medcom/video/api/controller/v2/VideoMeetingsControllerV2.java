...
public class VideoMeetingsControllerV2 implements VideoMeetingsV2Api {
    ...
    @Override
    @PreAuthorize(plannerProvisionerUserAdminUserRoleAtt)
    public ResponseEntity<List<Participant>> v2MeetingsUuidParticipantsCitizenPost(UUID uuid, List<CreateParticipant> createParticipant) {
        logger.debug("Enter POST citizen participants by meeting uuid: {}, v2.", uuid);
        try {
            var mappedParticipants = ParticipantMapper.externalToInternal(createParticipant);
            var participants = participantService.createCitizenParticipants(uuid, mappedParticipants);
            return ResponseEntity.ok(ParticipantMapper.internalToExternal(participants));
        } catch (PermissionDeniedExceptionV2 e) {
            throw new PermissionDeniedException(e.getMessage());
        } catch (ResourceNotFoundExceptionV2 e) {
